...
export class InstallerOwnerGuestStorageModalComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.edge.getFirstValidConfig(this.websocket).then((config) => {
            ...
            const chargeDischargeLimiterCtrl = this.config.getComponentsByFactory(
                "Controller.Ess.ChargeDischargeLimiter",
            );
            ...
            const components = [...prepareBatteryExtensionCtrl, ...emergencyReserveCtrl, ...chargeDischargeLimiterCtrl]
                .filter((component) => component.isEnabled)
                ...
                for (const essId of Object.keys(components)) {
                    ...
                    for (const controller of controllers as EdgeConfig.Component[]) {
                        if (controller.factoryId == "Controller.Ess.EmergencyCapacityReserve") {
                            ...
                            controllerFrmGrp.addControl(
                                "chargeDischargeLimiterController",
                                this.formBuilder.group({
                                    controllerId: new FormControl(controller["id"]),
    ...
    async applyChanges() {
        ...
        for (const essId in this.formGroup.controls) {
            ...
            const chargeDischargeLimiterController =
                (essGroups.get("chargeDischargeLimiterController") as FormGroup)?.controls ?? {};
            for (const essGroup of Object.keys(chargeDischargeLimiterController)) {
                if (chargeDischargeLimiterController[essGroup].dirty) {
                    if (updateArray.get(chargeDischargeLimiterController["controllerId"].value)) {
                        updateArray
                            .get(chargeDischargeLimiterController["controllerId"].value)
                            .push(new Map().set(essGroup, chargeDischargeLimiterController[essGroup].value));
                    } else {
                        updateArray.set(chargeDischargeLimiterController["controllerId"].value, [
                            new Map().set(essGroup, chargeDischargeLimiterController[essGroup].value),
                        ]);
                    }
                }
            }
